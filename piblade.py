...
while 1:
    ...
    # PI BLADE LOGIC
    SHOOT_ACCEL_POINT = 1.8
    if (math.fabs(accel_x) >= SHOOT_ACCEL_POINT or math.fabs(accel_y)  >= SHOOT_ACCEL_POINT or math.fabs(accel_z)  >= SHOOT_ACCEL_POINT):
        shoot_accel_remain_count += 1
    else:

        if (shoot_accel_remain_count > 5):
            print 'HTTP request okurimashita http://bayblade-final.herokuapp.com/put?putStr=' + str(shoot_accel_remain_count)
            requests.get('http://bayblade-final.herokuapp.com/put?putStr=' + str(shoot_accel_remain_count))

            print       # 改行
            print '============== RESULT   ' + str(shoot_accel_remain_count) + ' POWER !!!!! ============='
            sys.exit()

        shoot_accel_remain_count = 0
    print shoot_accel_remain_count
